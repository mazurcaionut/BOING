
/**
 * This class reads a shape file.  For the format of this shape file, see the assignment description.
 * Also, please see the shape files ExampleShapes.txt, ExampleShapesStill.txt, and TwoRedCircles.txt
 *
 * @author Ioan Mazurca
 *
 */

import javafx.scene.paint.Color;

import java.io.File;
import java.io.FileNotFoundException;
import java.util.Scanner;

public class ReadShapeFile {

    // TODO: You will likely need to write four methods here. One method to
    // construct each shape
    // given the Scanner passed as a parameter. I would suggest static
    // methods in this case.

    /**
     * Reads the data file used by the program and returns the constructed queue
     *
     * @param in
     *            the scanner of the file
     * @return the queue represented by the data file
     */


    /**
     * Reads the attributes of a triangle
     * @param in the scanner of the file
     * @return the triangle generated by those attributes
     */
    private static Triangle readTriangle(Scanner in){
        int x1=in.nextInt();
        int y1=in.nextInt();
        int vx=in.nextInt();
        int vy=in.nextInt();
        boolean isFilled=in.nextBoolean();
        int side=in.nextInt();
        int R=in.nextInt();
        int G=in.nextInt();
        int B=in.nextInt();
        boolean isPulsing=in.nextBoolean();
        int insertionTime=in.nextInt();
        Color colour=Color.rgb(R,G,B);

        Triangle triangle=new Triangle("triangle", insertionTime, x1, y1, vx, vy, side, colour, isFilled, isPulsing);

        return triangle;
    }

    /**
     * Reads the attributes of a rect
     * @param in the scanner of the file
     * @return the rect generated by those attributes
     */
    private static Rect readRect(Scanner in){
        int x=in.nextInt();
        int y=in.nextInt();
        int vx=in.nextInt();
        int vy=in.nextInt();
        boolean isFilled=in.nextBoolean();
        int width=in.nextInt();
        int height=in.nextInt();
        int R=in.nextInt();
        int G=in.nextInt();
        int B=in.nextInt();
        boolean isPulsing=in.nextBoolean();
        int insertionTime=in.nextInt();
        Color colour=Color.rgb(R,G,B);

        Rect rect=new Rect("rect", insertionTime, x,y,vx, vy, width, height,colour,isFilled, isPulsing);

        return rect;
    }
    /**
     * Reads the attributes of a square
     * @param in the scanner of the file
     * @return the square generated by those attributes
     */
    private static Square readSquare(Scanner in){
        int x=in.nextInt();
        int y=in.nextInt();
        int vx=in.nextInt();
        int vy=in.nextInt();
        boolean isFilled=in.nextBoolean();
        int side=in.nextInt();
        int R=in.nextInt();
        int G=in.nextInt();
        int B=in.nextInt();
        boolean isPulsing=in.nextBoolean();
        int insertionTime=in.nextInt();
        Color colour= Color.rgb(R,G,B);

        Square square=new Square("square", insertionTime, x, y, vx, vy, side, colour, isFilled, isPulsing);

        return square;
    }

    /**
     * Reads the attributes of a circle
     * @param in the scanner of the file
     * @return the circle generated by those attributes
     */
    private static Circle readCircle(Scanner in) {
        int x=in.nextInt();
        int y=in.nextInt();
        int vx=in.nextInt();
        int vy=in.nextInt();
        boolean isFilled=in.nextBoolean();
        int diameter=in.nextInt();
        int R=in.nextInt();
        int G=in.nextInt();
        int B=in.nextInt();
        boolean isPulsing=in.nextBoolean();
        int insertionTime=in.nextInt();
        Color colour= Color.rgb(R,G,B);

        Circle circle=new Circle("circle", insertionTime, x, y, vx, vy, diameter, colour, isFilled, isPulsing);

        return circle;
    }

    /**
     * Reads the attributes of an oval
     * @param in the scanner of the file
     * @return the oval generated by those attributes
     */
    private static Oval readOval(Scanner in) {
        int x=in.nextInt();
        int y=in.nextInt();
        int vx=in.nextInt();
        int vy=in.nextInt();
        boolean isFilled=in.nextBoolean();
        int width=in.nextInt();
        int height=in.nextInt();
        int R=in.nextInt();
        int G=in.nextInt();
        int B=in.nextInt();
        boolean isPulsing=in.nextBoolean();
        int insertionTime=in.nextInt();
        Color colour=Color.rgb(R,G,B);

        Oval oval=new Oval("oval", insertionTime, x,y,vx, vy, width, height,colour,isFilled, isPulsing);

        return oval;
    }

    /**
     * Read the shape files and place them on the Queue
     * @param in the scanner of the file
     * @return The queue that contains the shapes from the file
     */
    private static Queue<ClosedShape> readDataFile(Scanner in) {
        Queue<ClosedShape> shapeQueue = new Queue<ClosedShape>();

        //read in the shape files and place them on the Queue

        while(in.hasNext()) {
            String objectType=in.next();
            if(objectType.equals("circle")){
                //Create a circle object
                Circle circle=readCircle(in);
                //Place it at the back of the queue
                shapeQueue.enqueue(circle);
            }
            else if(objectType.equals("oval")){
                //Create an oval object
                Oval oval=readOval(in);
                //Place it at the back of the queue
                shapeQueue.enqueue(oval);
            }
            else if(objectType.equals("rect")) {
                //Create a rect object
                Rect rect = readRect(in);
                //Place it at the back of the queue
                shapeQueue.enqueue(rect);
            }
            else if(objectType.equals("square")) {
                //Create a square object
                Square square = readSquare(in);
                //Place it at the back of the queue
                shapeQueue.enqueue(square);
            }
            else if(objectType.equals("triangle")) {
                //Create a triangle object
                Triangle triangle = readTriangle(in);
                //Place it at the back of the queue
                shapeQueue.enqueue(triangle);
            }
        }


        //Right now, returning an empty Queue.  You need to change this.
        return shapeQueue;
    }

    /**
     * Method to read the file and return a queue of shapes from this file. The
     * program should handle the file not found exception here and shut down the
     * program gracefully.
     *
     * @param filename
     *            the name of the file
     * @return the queue of shapes from the file
     */
    public static Queue<ClosedShape> readDataFile(String filename) {
        // HINT: You might want to open a file here.

        //Create a new file which takes the String filename as a name
        File inputFile=new File(filename);
        Scanner in = null; //null is not sensible. Please change

        //Use a try and catch block to check if the file exists,
        // and if not, throws the FileNotFoundException
        try{
            in=new Scanner(inputFile);
        }
        catch (FileNotFoundException e){
            System.out.println("Cannot open "+filename);
            System.exit(0);
        }

        return ReadShapeFile.readDataFile(in);

    }
}

